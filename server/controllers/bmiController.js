...
export const saveBMI = async (req, res) => {
  try {
    ...
    console.log(
      `[BMI Controller] Saving BMI for user: ${userId}`
    );

    console.log(
      `[BMI Controller] Data: Weight=${weight}, Height=${height}, BMI=${bmi}, Status=${status}`
    );
    ...
    res.status(500).json({
      message: "Failed to save BMI record",
    });
  }
};
...
export const getBMIHistory = async (req, res) => {
  try {
    ...
    console.log(
      `[BMI Controller] Fetching BMI history for user: ${userId}`
    );
...
// ADMIN - GET ALL BMI HISTORY
export const getAllBMIHistory = async (req, res) => {
  try {
    console.log(
      `[BMI Controller] ADMIN fetching all BMI history...`
    );
    ...
    res.status(500).json({
      message: "Failed to fetch all BMI history",
    });
  }
};

// ADMIN - DELETE BMI RECORD
export const deleteBMIRecord = async (req, res) => {
  try {
    const recordId = req.params.id;

    console.log(
      `[BMI Controller] ADMIN deleting BMI record: ${recordId}`
    );
